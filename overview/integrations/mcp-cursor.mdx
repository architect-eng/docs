...
<AccordionGroup>
  ...
  <Accordion title="Server shows as disconnected">
    Check that the URL is exactly `https://mcp.dev.tryarchitect.com/mcp`. Try removing the server and re-adding it. If it still fails, restart Cursor.
  </Accordion>

  <Accordion title="Authentication window doesn't appear">
    Make sure your browser isn't blocking pop-ups from Cursor. Try opening `https://mcp.dev.tryarchitect.com/mcp` directly in your browser to verify the server is reachable.
  </Accordion>
</AccordionGroup>
